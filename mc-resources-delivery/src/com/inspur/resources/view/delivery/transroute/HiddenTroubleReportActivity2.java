...
public class HiddenTroubleReportActivity2 extends BaseActivity {
	...
	protected void onCreate(Bundle bundle) {
		...
		setContentView(R.layout.activity_hiddentroublereport_change);
		setTitle("隐患上报");
	...
	private void initUI() {
		...
		String uid = PreferencesUtils.getString(HiddenTroubleReportActivity2.this, "UID", "");
		...
		if(cType.equals(ApplicationValue.single_list[0]) || isUpdate){
			...
			jjcd_spinner.setOnItemSelectedListener(new OnItemSelectedListener() {
				...
				public void onItemSelected(AdapterView<?> parent, View view,int position, long id) {
					switch (position) {
						case 0:
							troubleSp1 = getResources().getStringArray(R.array.trouble_b_1);
							jjcdAdapter2 = ArrayAdapter.createFromResource(HiddenTroubleReportActivity2.this, R.array.trouble_b_1,
									android.R.layout.simple_dropdown_item_1line);
							break;
						case 1:
							troubleSp1 = getResources().getStringArray(R.array.trouble_b_2);
							jjcdAdapter2 = ArrayAdapter.createFromResource(HiddenTroubleReportActivity2.this, R.array.trouble_b_2,
									android.R.layout.simple_dropdown_item_1line);
							break;
						case 2:
							troubleSp1 = getResources().getStringArray(R.array.trouble_b_3);
							jjcdAdapter2 = ArrayAdapter.createFromResource(HiddenTroubleReportActivity2.this, R.array.trouble_b_3,
									android.R.layout.simple_dropdown_item_1line);
							break;
						case 3:
							troubleSp1 = getResources().getStringArray(R.array.trouble_b_4);
							jjcdAdapter2 = ArrayAdapter.createFromResource(HiddenTroubleReportActivity2.this, R.array.trouble_b_4,
									android.R.layout.simple_dropdown_item_1line);
							break;
					}
			...
			iv_time.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					new DatePickerDialog(HiddenTroubleReportActivity2.this,
							// 绑定监听器
			...
			//数据不准确资源类别
			ArrayAdapter jjcdAdapter5 = ArrayAdapter.createFromResource(HiddenTroubleReportActivity2.this, R.array.data_erro_type,
					android.R.layout.simple_dropdown_item_1line);
			...
			iv_wang.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					if(!isKong(company_edit.getText().toString()) && ! isKong(qy_edit.getText().toString())){
						Intent gIntent = new Intent(HiddenTroubleReportActivity2.this,DeliveryResourceSearchListActivity2.class);
						...
						Toast.makeText(HiddenTroubleReportActivity2.this, "请先选择地市和区县", Toast.LENGTH_SHORT).show();
					}
				}
			});
			//数据不准确段落名称
			...
			iv_start_jing.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					Intent gIntent = new Intent(HiddenTroubleReportActivity2.this,JingSelectActivity.class);
			...
			iv_end_jing.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					if(JingStart == null){
						Toast.makeText(HiddenTroubleReportActivity2.this, "请先选择起点井", Toast.LENGTH_SHORT).show();
						return;
					}
					Intent gIntent = new Intent(HiddenTroubleReportActivity2.this,JingSelectActivity.class);
					//	Intent gIntent = new Intent(HiddenTroubleReportActivity2.this,MapTemp.class);
	...
	private void initData(ErrorInfoBean errorInfoBean) {
		...
		if(cType.equals(ApplicationValue.single_list[0]) || isUpdate){
			...
			yhms_edit.setText(errorInfoBeanZz.getErrorDescribe());  //隐患描述
			...
			yhms_edit.setText(errorInfoBeanGd.getErrorDescribe());  //隐患描述
	...
	private ErrorInfoBean editInfo(ErrorInfoBean errorInfo) {
		...
		errorInfoBean.setZgRq(MyTimeUtils.getCurrentTime());//整改日期
		errorInfoBean.setRouteID(routeID);
	...
	private ErrorInfoZzBean editInfoZz(ErrorInfoZzBean errorInfo) {
		...
		errorInfoBeanZz.setErrorDescribe(yhms_edit.getText().toString());  //隐患描述
		...
		errorInfoBeanZz.setZgRq(MyTimeUtils.getCurrentTime());	//整改日期
		errorInfoBeanZz.setRouteID(routeID);
	...
	private ErrorInfoGdgzBean editInfoGd(ErrorInfoGdgzBean errorInfo) {
		...
		errorInfoBeanGd.setErrorDescribe(yhms_edit.getText().toString());  //隐患描述
		errorInfoBeanGd.setRouteID(routeID);
		if(JingStart != null){
	...
	public void yhdx_onc(View v) {
		startActivityForResult(new Intent(HiddenTroubleReportActivity2.this, DeliveryResourceSearchListActivity.class), REQUESTCODE_GET_RESOURCE);
	}

	// 拍照
	public void cs_take_photo(View v) {
		if(resourceInfoBean == null){
			Toast.makeText(HiddenTroubleReportActivity2.this, "程序异常，请重新进入!", Toast.LENGTH_SHORT).show();
			return;
		}
		startPhotoBacked = false;
		Intent intent = new Intent(HiddenTroubleReportActivity2.this, PhotoShopeActivityGuang.class);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				final String httpGetData = new httpconnect().httpGetData("pdaMainTask!getPhotoById.interface", list,
						HiddenTroubleReportActivity2.this);
	...
	private void upload(){
		...
		if(isKong(qy_edit.getText().toString())){
			Toast.makeText(HiddenTroubleReportActivity2.this, "请选择区县", Toast.LENGTH_SHORT).show();
			return;
		}
		...
		if(cType.equals(ApplicationValue.single_list[0])){
			...
			if(isKong(sbr_edit.getText().toString())){
				Toast.makeText(HiddenTroubleReportActivity2.this, "请填写隐患位置描述!", Toast.LENGTH_SHORT).show();
				return;
			}

			if(isKong(yhms_edit.getText().toString())){
				Toast.makeText(HiddenTroubleReportActivity2.this, "请填写隐患描述", Toast.LENGTH_SHORT).show();
				return;
			}
			...
			if(str_isChange.equals("是")){
				/*if(isKong(tv_time.getText().toString())){
					Toast.makeText(HiddenTroubleReportActivity2.this, "是否整改选择是时，整改日期必填", Toast.LENGTH_SHORT).show();
					return;
				}*/
			}else{

				errorInfoBean.setZgRq("");
			}
			...
			if(isKong(tv_wang.getText().toString())){
				Toast.makeText(HiddenTroubleReportActivity2.this, "请选择数据不准确网元名称!", Toast.LENGTH_SHORT).show();
				return;
			}

			if(isKong(tv_guang.getText().toString())){
				Toast.makeText(HiddenTroubleReportActivity2.this, "请选择数据不准确段落名称!", Toast.LENGTH_SHORT).show();
				return;
			}
			errorInfoBeanZz = editInfoZz(errorInfoBeanZz);
			if(str_isChange.equals("是")){
				/*if(isKong(tv_time.getText().toString())){
					Toast.makeText(HiddenTroubleReportActivity2.this, "是否完成数据修改选择是时，数据修改日期必填", Toast.LENGTH_SHORT).show();
					return;
				}*/
			} else{
				errorInfoBeanZz.setZgRq("");
			}

			jsonRequest = new GsonBuilder().setDateFormat("yyyy-MM-dd HH:mm:ss Z").create().toJson(errorInfoBeanZz);
			Log.d("lixu","jsonRequest == "+jsonRequest.toString());
		}else if(cType.equals(ApplicationValue.single_list[2])){
			saveUrl = "saveErrorGdgzInfo";
			if(isKong(tv_start_jing.getText().toString())){
				Toast.makeText(HiddenTroubleReportActivity2.this,"请选择起点井",Toast.LENGTH_SHORT).show();
				return;
			}
			if(isKong(tv_end_jing.getText().toString())){
				Toast.makeText(HiddenTroubleReportActivity2.this,"请选择终点井",Toast.LENGTH_LONG).show();
				return;
			}

			if(isKong(et_lenge.getText().toString())){
				Toast.makeText(HiddenTroubleReportActivity2.this,"请填写故障长度(米)",Toast.LENGTH_SHORT).show();
				return;
			}
		...
		new Thread(new Runnable() {
			@Override
			public void run() {
				final String httpGetData = new httpconnect().httpGetData("pdaMainTask!"+saveUrl+".interface", list,
						HiddenTroubleReportActivity2.this);
				try {
					final Message message = new Message();
					final JSONObject jsonObject = new JSONObject(httpGetData.toString());
	...
	private Handler handler = new Handler(new Handler.Callback() {
		...
		public boolean handleMessage(Message msg) {
			...
			switch (msg.what) {
				...
				case -1:
					Toast.makeText(HiddenTroubleReportActivity2.this, msg.obj.toString(), Toast.LENGTH_SHORT).show();
					break;
				case 4:
					Toast.makeText(HiddenTroubleReportActivity2.this, msg.obj.toString(), Toast.LENGTH_SHORT).show();
				...
				case 9:
					...
					if(list!=null && list.size()!=0){
						...
						Toast.makeText(HiddenTroubleReportActivity2.this, "请先上传图片", Toast.LENGTH_SHORT).show();
					}
					break;
				case 10:
					Toast.makeText(HiddenTroubleReportActivity2.this, msg.obj.toString(), Toast.LENGTH_SHORT).show();

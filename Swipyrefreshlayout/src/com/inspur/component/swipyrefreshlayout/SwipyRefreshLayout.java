...
//                if (!isEnabled() || mReturningToStart || (!mBothDirection && canChildScrollDown()) || mRefreshing) {
            	if (!enableBottom || mReturningToStart || (!mBothDirection && canChildScrollDown()) || mRefreshing) {
...
//                if (!isEnabled() || mReturningToStart || (!mBothDirection && canChildScrollUp()) || mRefreshing) {
            	 if (!enableTop || mReturningToStart || (!mBothDirection && canChildScrollUp()) || mRefreshing) {
                       // Fail fast if we're not in a state where a swipe is possible
                    return false;
                }
                break;
...
//                    if (!isEnabled() || mReturningToStart || canChildScrollDown() || mRefreshing) {
                	 if (!enableBottom || mReturningToStart || canChildScrollDown() || mRefreshing) {
                           // Fail fast if we're not in a state where a swipe is possible
                        return false;
                    }
                    break;
...
//                    if (!isEnabled() || mReturningToStart || canChildScrollUp() || mRefreshing) {
                    	if (!enableTop || mReturningToStart || canChildScrollUp() || mRefreshing) {
                                // Fail fast if we're not in a state where a swipe is possible
                        return false;
                    }
                    break;
